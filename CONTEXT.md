...
**Stage**:
A boss-gated segment of a **Dungeon** — and its own **instanced run**. The **Delve** chains Stages **without end** (ADR-0015): two authored ones, then the generated **Depths**. Stage 1 (the mine-into-ruins), entered from the World shaft, ends at the **Deep Guardian**, whose defeat pays out that run's participation loot, shakes the room, and opens a hidden door. Any Player presses interact at the door to start **Stage 2 — the Deep**, a molten forge-depth: a *fresh* instanced run in a new magma interior, ending at **the Forgeborn** — whose fall opens the next door in turn, into the first generated **Depth** (Stage 3, and so on forever). The Deep's roster is the players who **descend** (Exhausted or absent ones drop; no one new joins — the door is reachable only from inside, so the roster can only shrink), it scales to that headcount, and its **host is the descent's initiator** (a fresh sim, never a migration). The door is an *in-Dungeon* entrance, not a second World entrance, so every run must clear Stage 1 to reach the Deep (no skip-ahead). Each Stage pays its **own** participation loot the moment its boss falls — one DB write per Stage — so a Deep wipe never costs the banked Stage-1 reward; descent is one-way and optional (a party may instead leave with its Stage-1 loot).
_Avoid_: level, floor, room (the loading/gameplay-shape words the Dungeon entry already forbids)

**Descent**:
One party's unbroken chain of Stage runs down the **Delve** in a single sitting — from the World shaft as deep as its roster keeps clearing. The roster can only shrink, and a solo Descent is simply a party of one. It ends by wipe, by walking out, or by declining the next door; every cleared Stage's loot stays banked regardless. Its **Depth** is what a **Depth Record** remembers.
_Avoid_: run (that is one Stage's instance; the Descent is the chain of them), attempt, session

**Depth**:
How deep a **Descent** got: the count of Stage bosses felled (Deep Guardian = 1, the Forgeborn = 2, the endless ladder = 3+). A Depth-3+ Stage is **generated, not authored**: its palette, Husk family (names composed from localized word lists), boss kit, floor plan (constrained to the authored grammar — safe entry chamber → rooms west-to-east → boss room) and tuning are a **pure function of the Depth number** — identical in every run and every World, no seed on the wire (ADR-0015) — and difficulty compounds without end, so the wall always wins eventually. A Player is credited a Depth exactly when they earn that Stage's participation loot.
_Avoid_: floor, level (already forbidden at Dungeon), depth-as-checkpoint (there is none — every Descent starts at Stage 1)

**Depth Sigil**:
The only loot of the generated Depths (3+): one per Stage boss, paid by the participation rule. Pure prestige — it crafts nothing combat-relevant; its sinks are a large **Village**-pool contribution value and (a later pass) trophy decor. The real prize of the deep ladder is the **Depth Record** itself.
_Avoid_: currency, token, power loot

**Depth Record**:
A World's memory of its **Descents**: the deepest-Descents board (Depth, roster, date) plus each Player's personal best (the deepest Stage *they* helped clear) — written in the same single DB write that pays a Stage's participation loot, scoped per World, never pruned (the board merely displays a top slice). Displayed at the **Grand Monument** (interact → the engraved board) with a one-line current-record teaser on the **Hall** panel; tracked from the first Descent even while the Monument is unbuilt. The game's first *individual* display — allowed because it is pure prestige: no power, no gating, and the Village pool stays collective-only.
_Avoid_: leaderboard, highscore (competitive framings — a Record celebrates feats, solo or party alike)

**Husk**:
A reactive creature that lives **only inside a Dungeon** — animated constructs stirred by the ruins, not living beings, so the open **World**'s only loose creatures are natural **Wildlife** (ADR-0012), never Husks. Every **Stage** holds two kinds (a melee chaser, a ranged kiter) and ends at a scaled-up Husk **boss**. In the **Delve**, Stage 1 is **stone-and-clay** (a Grasp Husk + a Spit Husk, ending at the **Deep Guardian**); Stage 2 (the **Deep**) is **cinder-and-basalt** (a Cinder Husk + an Ember Husk, ending at **the Forgeborn**); the generated **Depths** (3+) re-dress the same two archetypes and recycle the two boss kits per Depth, palette/name/tuning from the pure per-Depth function (ADR-0015). All are the same reactive AI — reskinned and retuned, never a new engine. Husks chase, telegraph their attacks, and knock Players down (never kill); the party's **host** client simulates them, never a server.
...
- Cooking: fish can be cooked at any campfire **Structure** (giving campfires their function). Eating a cooked fish grants ~+20% move speed for ~3 minutes — a temporary, individual **food buff** (once the game's *only* buff). Future fish species/collections are a noted extension, not in scope now. (History: a tiki-statue aura buff was rejected 2026-07 under the old one-buff rule; that rule is now **retired** — the **Village** grants collective per-tier combat buffs, ADR-0013 — and the food buff stacks on top.)
...
- The **Delve**'s chain is endless (ADR-0015): past the authored Stage 2, each boss's fall opens another door into a generated **Depth** — deterministically themed per Depth number, compounding in difficulty, paying only **Depth Sigils** — and every Stage clear writes the World's **Depth Record** in the same call that pays its participation loot. There are no checkpoints: a record Depth is earned in one sitting, from the shaft down.
- The **Village** is the game's **meta-loop**: gathering, the **Guardian**, the **Dungeon**, and the **frontier** all yield contributions that feed its **Hall**, raising the communal **tier**, which unlocks more to build — the reason to go back out (ADR-0010). It runs **parallel** to the tier-1→tier-2 tool ladder and **gates nothing**; each tier unlocks new **Buildings**, visible grandeur, **and a collective combat buff** — move/attack speed + crit, shared by every Player (ADR-0013, which **retires the one-buff rule**). Goals stay **implicit** — the Village's visible grandeur and the Hall's pool display *are* the signpost (the resolution of "quests vs. implicit goals": keep it implicit, let the World dangle the goal).
...
- **Hunting/foraging feeds existing loops, adds no power.** Wildlife drops a **hide / meat / trophy** Resource family that flows only into things that already exist: the **Village** pool (the "frontier finds" ADR-0010 anticipated), **cooking** (a new campfire *ingredient* granting the same move-speed **food buff** — a new input, not a new buff), and **decor / trophy Structures** (cozy expression + Village grandeur). Wildlife loot itself grants **no armour, no weapon stats, no direct buff** — the **no-HP rule is inviolate**; it only *feeds* the **Village** pool, whose **tiers** now grant collective combat buffs (ADR-0013, retiring the one-buff rule). The **Village still gates nothing**.
- **Village Buildings earn their footprint (ADR-0013 building-function passes).** Beyond the Hall's contribution and tier grandeur, milestone/decor Buildings gained interactions: the **Market Square** is a resource **Trade Post** (tier-taxed swap of surplus raws), the **Banner** names the Village + picks a crest, the **Well** keeps a short **Chronicle**, and the **Fountain** is a communal **Wishing Well** — Players toss **fruit** into a shared meter and, at threshold, a **Dorffest** runs a few real minutes granting **everyone** a small **move-speed** boost (a fresh timestamp mechanic, ADR-0001/0002). The **Grand Monument** — until ADR-0015 the last interaction-less Building — is the **Depth Record** stone (interact → the engraved deepest-Descents board). These add utility but still **gate nothing** and never introduce HP/loss.

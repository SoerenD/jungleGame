...
**Structure**:
A crafted object placed into the World, by footprint either a **Building** (multi-tile, ≥2×2 — hut, workshop, house; the makings of a village) or a **Prop** (1-tile — torch, signpost, statue, campfire). Placement claims every tile of its footprint, and any Structure can be **dismantled** (removed) — Structures are no longer permanent.
_Avoid_: placeable

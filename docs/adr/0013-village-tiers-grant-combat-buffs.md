...
- **Building functions are a separate, additive line** (referenced as "ADR-0013 building-function passes" in commits, distinct from the buff decision above). The milestone/decor Buildings gained interactions: Market **Trade Post** (migration `0007`), Banner **Name & Crest** + Well **Chronicle** (`0008`), and the Fountain reworked from a read-only Trophy into a communal **Wishing Well → Dorffest** (`0009`: toss fruit → shared meter → at 30 a 5-min world-wide **+10% move** buff, a pure `festivalUntil` timestamp). Unlike the tier buffs, these **do** add persistent `village` jsonb state + additive RPCs — but stay client-authoritative, mid-play-safe, and gate nothing. Tuning: `FOUNTAIN_WISH_THRESHOLD` / `FESTIVAL_MS` / `FESTIVAL_SPEED_FACTOR` in `content/village.ts` (threshold mirrored in the SQL).

...
Don't couple `CONTEXT.md` to implementation details. Only include terms that are meaningful to
domain experts.
...
When the grilling is complete and all branches are resolved, write a decision summary to
`.claude/plans/feature-plan-<feature-name>.md`. This file bridges the grilling session to
`/implement-feature` — it survives compaction, crashes, and session boundaries.

Include:

1. **Feature name and one-line description**
2. **Resolved decisions** — every design choice made during the session, stated as facts
3. **Acceptance criteria** — Given/When/Then conditions that define when the feature is done.
   These feed directly into integration tests and the self-healing test loop
4. **Scope boundaries** — what NOT to build. Explicitly list adjacent areas that might seem
   related but are out of scope (e.g., "do NOT refactor the existing notification system")
5. **Constraints and gotchas** — anything that would surprise an implementer
6. **References** — links to CONTEXT.md terms and ADRs created during this session

Keep it implementation-ready: facts and decisions, not discussion history. The architect agent
will read this file and use it as the basis for a technical plan.

Ask the user to confirm the summary before writing it.
